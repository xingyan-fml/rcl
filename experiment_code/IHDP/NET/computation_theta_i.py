...
def compute_ATE(ATE_df, exp_df, num_D, max_r, model_net_methods):
    '''compute ATE'''
    '''compute true ATE'''
        ...
        for j in range(i + 1, num_D):
            ...
            for model_name in model_net_methods:
                for estimator in ['DR', 'IPW', 'AIPW', 'DML', 'trim_DML']:
                    ...
                    ATE_df.loc[model_name, estimator + '_' + str(i + 1) + ',' + str(j + 1)] = \
                        exp_df.loc[model_name, estimator + '_' + str(i + 1)] - exp_df.loc[
                            model_name, estimator + '_' + str(j + 1)]
                    ATE_df.loc[model_name + '_e', estimator + '_' + str(i + 1) + ',' + str(j + 1)] = \
                        np.abs(
                            ATE_df.loc[model_name, estimator + '_' + str(i + 1) + ',' + str(j + 1)] /
                ...
                for r in range(2, max_r + 1):
                    for k in range(1, r + 1):
                        ATE_df.loc[model_name, str(r) + '&' + str(k) + '_' + str(
                            i + 1) + ',' + str(j + 1)] = \
                            exp_df.loc[
                                model_name, str(r) + '&' + str(k) + '_' + str(i + 1)] - \
                            exp_df.loc[
                                model_name, str(r) + '&' + str(k) + '_' + str(j + 1)]
                        ATE_df.loc[
                            model_name + '_e', str(r) + '&' + str(k) + '_' + str(
                                i + 1) + ',' + str(j + 1)] = \
                            np.abs(ATE_df.loc[
                                       model_name, str(r) + '&' + str(k) + '_' + str(
